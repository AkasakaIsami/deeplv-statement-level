...
from util import float_to_percent, transact, OR2OEN, tensor2label, classacc, AOD, auROC


def train_and_test(dataset: MyDataset):
    # 第一步：训练配置
    # 不支持批处理！！
    BS = 5
    LR = 5e-3
    ...
    def my_collate_fn(batch):
        ...
        for data in batch:
            ...
            ys.append(data[1])
            ids.append(data[2])
            max_len = data[0].shape[0] if data[0].shape[0] > max_len else max_len
    ...
    acc = accuracy_score(y_total.cpu(), y_hat_total.cpu())
    auc = auROC(y_total.cpu(), y_float_hat_total.cpu())
    class_acc = classacc(y_total.cpu(), y_hat_total.cpu())
    aod = AOD(y_total.cpu(), y_hat_total.cpu())
